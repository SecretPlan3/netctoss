...
public class BillTimeDaoImpl  extends BaseDao implements IBillTimeDao{
	...
	public PagerBean findAllServicesBillTimeByCondition(PagerBean pb) {
		// TODO Auto-generated method stub
		StringBuilder hql = new StringBuilder( "from ServiceBean as s   left join fetch s.cost as c  left join fetch s.user as u   left join fetch s.serviceYear as y    left join fetch y.serviceMonthly as m     left join fetch m.serviceDaily as d  where 1=1"); 
		...
		// 得到数据总数
		hql = new StringBuilder( "select count(DISTINCT s.id) from ServiceBean as s   left join  s.cost as c  left join  s.user as u   left join  s.serviceYear as y    left join  y.serviceMonthly as m     left join  m.serviceDaily as d  where 1=1");
		if (pb.getParams().get("year") != null && ! pb.getParams().get("year").equals("") ) {
			hql.append(	" and y.year = :year ");
		}
		if (pb.getParams().get("month") != null && ! pb.getParams().get("month").equals("")  ) {
