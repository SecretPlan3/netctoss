...
public class LogmagDaoImpl extends BaseDao implements ILogmagDao {
	...
	public PagerBean findManagerLog(PagerBean pager) {
		...
		//根据管理查询登陆/退出系统时间
		StringBuilder hql = new StringBuilder("select count(l.id)from ManagerLogBean as l where l.fk_manager_id = :managerId");
		...
		hql = new StringBuilder("from ManagerLogBean as l where l.fk_manager_id = :managerId");
	...
	public PagerBean findManagerOperationLog(PagerBean pager) {
		// TODO Auto-generated method stub
		StringBuilder hql = new StringBuilder("select count(l.id)from ManagerOperationLogBean as l where 1 = 1");
		if(pager.getParams().get("managerId") != null) {
			//根据管理员查询该管理操作信息
			hql.append(" and l.fk_manager_id = :managerId");
		}
		...
		hql = new StringBuilder("from ManagerOperationLogBean as l where 1 = 1");
		if(pager.getParams().get("managerId") != null) {
			//根据管理员查询该管理操作信息
			hql.append(" and l.fk_manager_id = :managerId");
		}
